// Converts a single chunk of PURE sinhala text (no punctuation / spaces)
function _convertSinhalaChunk(text) {
  text = text.replace(/,/g, "￦");
  text = text.replace(/\./g, "");
  text = text.replace(/\(/g, "￫");
  text = text.replace(/\)/g, "￩");
...
/**
 * Main export: converts Unicode Sinhala text to FM Abhaya encoding.
 * English letters, numbers, spaces, newlines and punctuation are preserved.
 */
export function convertToFM(input) {
  if (!input) return "";

  // Split on runs of non-Sinhala characters and only convert Sinhala parts.
  // Sinhala Unicode block: U+0D80–U+0DFF  plus ZWJ U+200D
  const sinhalaRun = /[\u0D80-\u0DFF\u200D]+/g;
  return input.replace(sinhalaRun, (match) => _convertSinhalaChunk(match));
}
